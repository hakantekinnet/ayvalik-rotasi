      ...
      >
        ...
        <div className="p-2.5 xl:p-3 flex flex-col flex-1">
          ...
          <div className="mt-auto flex flex-col gap-1.5">
            {/* "Detayları Gör" — primary CTA */}
            <Link
              href={`/event/${evt.slug || evt._id}`}
              className="w-full text-[11px] font-bold py-2 rounded-xl bg-aegean-50 text-aegean-700 border border-aegean-100 hover:bg-aegean-100 transition-colors flex items-center justify-center gap-1 cursor-pointer"
            >
              <Eye size={12} />
              Detayları Gör
            </Link>
    ...
    <div className="w-full">
      {sortedEvents.length > 0 ? (
        <>
          ...
          <div className="xl:hidden flex overflow-x-auto snap-x snap-mandatory gap-4 pb-4 hide-scrollbar items-stretch -mx-4 px-4">
            {sortedEvents.map((evt) => (
              <div key={evt._id} className="w-[85vw] sm:w-[320px] shrink-0 snap-start">
                {renderEventCard(evt)}
              </div>
            ))}
          </div>
